...
export function ShowcaseCard({ imageSrc, logoSrc, title, avatars, category, platform, slug, link }: ShowcaseCardProps) {
                        ...
                        <img
                            ...
                            onError={(e) => {
                                ...
                                if (parent) {
                                    ...
                                    parent.style.background = 'linear-gradient(135deg, #1E2548 0%, #2D1B69 100%)';
                                }
                            }}
                        />
                    ) : (
                        <div className="w-full h-full bg-gradient-to-br from-[#1E2548] to-[#2D1B69] flex items-center justify-center">
                            <span className="text-white/20 text-5xl font-bold select-none">{title?.charAt(0) ?? '?'}</span>
                        </div>
                    )}
    ...
    if (internalHref) {
        return <Link to={internalHref} className={cardClass}>{inner}</Link>;
    }
    if (externalHref) {
        return <a href={externalHref} target="_blank" rel="noopener noreferrer" className={cardClass}>{inner}</a>;
    }
